...
const SURVEY_BASE = 'https://burn001.github.io/auri-ai-survey';
let ADMIN_TOKEN = '';
...
async function loadDashboard() {
  ...
  // by_category: Q6 자기응답 기준 응답 분포 (응답수 내림차순 list)
  const cats = Array.isArray(data.by_category) ? data.by_category : [];
  const totalRewardConsented = data.total_reward_consented || 0;
  const totalQuota = data.total_quota || 0;

  document.getElementById('stat-cards').innerHTML = `
    <div class="stat-card"><div class="label">전체 대상자</div><div class="value">${data.total_participants}</div></div>
    <div class="stat-card">
      <div class="label">응답 완료</div>
      <div class="value">${data.total_responses}</div>
      ${totalQuota ? `<div class="sub" style="font-size:11px;color:#6b7280;margin-top:4px">사례품 정원 ${totalRewardConsented} / ${totalQuota}</div>` : ''}
    </div>
  ...
  // 직군 분류는 응답자 Q6 자기응답 단일 출처. 막대 길이는 응답수 최댓값 대비 상대 폭,
  // 라벨의 % 는 전체 응답 대비 비율.
  const maxResp = cats.reduce((m, d) => Math.max(m, d.responded || 0), 0) || 1;
  const rows = cats.map(d => {
    const c = d.category;
    const pct = d.pct != null ? d.pct : 0;
    const bar = (d.responded / maxResp * 100).toFixed(0);
    const consented = d.reward_consented || 0;
    const quotaSub = d.quota
      ? `<span class="cat-quota" style="font-size:11px;color:#6b7280;margin-left:8px">사례품 ${consented}/${d.quota}${consented >= d.quota ? ' · 마감' : ''}</span>`
      : '';
    return `<div class="cat-row">
        <span class="cat-label">${c}</span>
        <div class="cat-track"><div class="cat-fill" style="width:${bar}%;background:${CAT_COLORS[c] || '#aaa'}"></div></div>
        <span class="cat-count">${d.responded}명 (${pct}%)${quotaSub}</span>
      </div>`;
  }).join('');

  const uncat = data.uncategorized
    ? `<div class="cat-row" style="opacity:.6">
        <span class="cat-label">미분류</span>
        <div class="cat-track"></div>
        <span class="cat-count">${data.uncategorized}명 (Q6 없음)</span>
      </div>`
    : '';

  document.getElementById('cat-bars').innerHTML =
    '<h3 style="font-size:14px;font-weight:600;margin-bottom:4px">직군별 응답 분포 <span style="font-weight:400;color:#6b7280;font-size:12px">(Q6 자기응답 기준)</span></h3>'
    + '<p style="font-size:11px;color:#9ca3af;margin:0 0 12px">초대 분류가 아닌 응답자 본인 선택(Q6) 기준 · 비율은 전체 응답 대비</p>'
    + rows + uncat;
}
...
let THREADS_CACHE = {};
let THREAD_DRAFTS = {};
let THREAD_REPLY_DRAFTS = {};
let THREAD_OPEN_REPLY = null;
let THREAD_OPEN_EDIT = null;
  ...
  const meta = `
    <div class="resp-meta">
      <dl>
        ...
        <dt>직군</dt><dd><span class="badge badge-blue">${escapeHtml(row.category || '-')}</span></dd>
