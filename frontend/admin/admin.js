...
async function loadDashboard() {
  ...
  const quotas = data.quota_per_category || {};

  // 사례품 정원 총 동의 카운트 (대시보드 stat 카드 보조 표시용)
  const totalRewardConsented = Object.values(cats).reduce((sum, d) => sum + (d.reward_consented || 0), 0);
  ...
  document.getElementById('cat-bars').innerHTML = '<h3 style="font-size:14px;font-weight:600;margin-bottom:12px">직군별 응답 현황</h3>' +
    CAT_ORDER.filter(c => cats[c]).map(c => {
      ...
      // 4직군만 사례품 정원이 존재. 그 외(연구진/기타/미분류)는 보조 라벨 생략.
      const quotaSub = q
        ? `<span class="cat-quota" style="font-size:11px;color:#6b7280;margin-left:8px">사례품 ${consented}/${q}${consented >= q ? ' · 마감' : ''}</span>`
        : '';
      return `<div class="cat-row">
        <span class="cat-label">${c}</span>
        <div class="cat-track"><div class="cat-fill" style="width:${pct}%;background:${CAT_COLORS[c] || '#aaa'}"></div></div>
        <span class="cat-count">${d.responded} / ${d.participants} (${pct}%)${quotaSub}</span>
      </div>`;
    }).join('');
}
